...
int handleOp(Operand op, FILE* fp, int load) {
    if (op->kind == VARIABLE_OP || op->kind == TEMP_VAR_OP || op->kind == CONSTANT_OP)
        ...
        // 写入加载指令 将从寄存器 reg 指向的内存地址加载数据到寄存器 reg 本身
        ...
        // // 检查变量描述符
        // if (var == NULL){
        //     printf("Error: Variable should have a descriptor.\n");
        //     exit(1);
        // }
        // 将计算后的变量地址装载到寄存器中 地址是相对于帧指针$fp的偏移量
        // 栈向下增长 因此var->offset需要取反
        // 使用addi指令 计算地址
...
void printObjectCodes(char* name) {
    ...
    while (flag == 1 || curr != interCodes) {
        ...
        switch (curr->kind) {
            ...
            case ASSIGN_IR: {
                // TODO
                Operand left = curr->ops[0];   // 左侧操作数x
                Operand right = curr->ops[1];  // 右侧操作数y
                // 将右侧操作数y的值装载到寄存器
                int regRight = handleOp(right, fp, 1);
                // 根据左侧操作数的类型进行分支
                if (left->kind == VARIABLE_OP || left->kind == TEMP_VAR_OP) {
                    // 如果左侧操作数是一个变量或临时变量，为left分配一个寄存器（不加载值）
                    int regLeft = getReg(left, fp, 0);
                    // 使用 move 指令将右侧操作数的值赋给左侧操作数 对应中间代码 x := y  ->  MIPS32指令move reg(x),reg(y)
                    fprintf(fp, "  move %s, %s\n", regs[regLeft]->name, regs[regRight]->name);
                    spillReg(regs[regLeft], fp);                
                } else if (left->kind == GET_VAL_OP) {
                    // 如果左侧操作数是间接赋值类型（解引用） 则要加载指针指向的地址
                    int regLeft = getReg(left->opr, fp, 1);
                    // 使用sw指令 将值存储到内存位置 对应中间代码 *x = y -> MIPS32指令sw reg(y),0(reg(x))
            ...
            case SUB_IR: {
                ...
                int regRight2 = handleOp(right2, fp, 1);
                if (left->kind == VARIABLE_OP || left->kind == TEMP_VAR_OP) {
                    // 如果结果变量left是一个变量或临时变量 分配一个寄存器(不加载值)
                    int regLeft = getReg(left, fp, 0);
                    // 减法指令 计算结果 存储在之前分配的寄存器中
                    ...
                    // 如果结果变量left是解引用类型
                    int regLeft1 = getReg(left->opr, fp, 0);
                    // 生成减法指令 计算结果 并存储在之前分配的寄存器regLeft1中 进行寄存器的值覆盖
                    fprintf(fp, "  sub %s, %s, %s\n", regs[regLeft1]->name, regs[regRight1]->name, regs[regRight2]->name);
                    // 如果左侧操作数是间接赋值类型（解引用） 则要加载指针指向的地址
                    int regLeft2 = getReg(left->opr, fp, 1);
                    // 使用sw指令 将值存储到内存位置 对应中间代码 *x = y -> MIPS32指令sw reg(y),0(reg(x))
            ...
            case DIV_IR: {
                // TODO
                // 参考MUL_IR进行实现 对应中间代码 x := y / z
                // 从中间代码提取操作数
                Operand left = curr->ops[0];  // 结果变量x
                Operand right1 = curr->ops[1];  // 被除数y
                Operand right2 = curr->ops[2];  // 除数x
                // 将操作数装载到寄存器
                int regRight1 = handleOp(right1, fp, 1);
                int regRight2 = handleOp(right2, fp, 1);
                // 生成除法指令 div reg(y),reg(z) 结果存储在特殊寄存器 lo（商）和 hi（余数）中
                fprintf(fp, "  div %s, %s\n", regs[regRight1]->name, regs[regRight2]->name);
                // 根据结果变量x的类型进行分支情况划分
                if (left->kind == VARIABLE_OP || left->kind == TEMP_VAR_OP) {   
                    // 如果结果变量left是一个变量或临时变量 需要为left分配一个寄存器
                    int regLeft = getReg(left, fp, 0);
                    // 使用 mflo 指令从 lo 寄存器中提取商，并将其存储在 left 对应的寄存器中  mflo reg(x)  
                    fprintf(fp, "  mflo %s\n", regs[regLeft]->name);
                    // 将可操作寄存器中的变量保存到栈上 
                    spillReg(regs[regLeft], fp);
                }
                    ...
                    // 如果结果变量left是解引用类型  
                    int regLeft1 = getReg(left->opr, fp, 0);
                    // 使用 mflo 指令提取商，并将其存储在 regLeft1 寄存器中
                    fprintf(fp, "  mflo %s\n", regs[regLeft1]->name);
                    // 为 left->opr 分配另一个寄存器 regLeft2 
                    int regLeft2 = getReg(left->opr, fp, 1);
                    // 使用 sw 指令将商存储到 regLeft2 指向的内存地址 
                    // 进行*x=y  <=>  sw reg(y),0(reg(x))   regLeft2加载值的寄存器 即为*x
                    fprintf(fp, "  sw %s, 0(%s)\n", regs[regLeft1]->name, regs[regLeft2]->name);
                }
                break;
            }
