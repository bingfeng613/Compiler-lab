...
InterCode translateExp(Node* root, Operand place) {
    ...
    if (root->childNum == 3 && strcmp(root->children[1]->name, "ASSIGNOP") == 0) {
        ...
        */
        if (root->children[0]->childNum == 1 && 
            ...
            // 根据变量名创建一个新的变量操作数tmp1
            Operand tmp1 = getVar(sym->name);
            // 创建一个新的临时操作数tmp2 存储右侧表达式的计算结果
            Operand tmp2 = newTemp();
            // 翻译右侧表达式 将结果存储于tmp2中
            InterCode code1 = translateExp(root->children[2], tmp2);
            // 创建新的赋值指令 将tmp2的值赋给tmp1
            InterCode code2 = (InterCode)malloc(sizeof(InterCode_));
            code2->kind = ASSIGN_IR; // 赋值指令类型
            code2->ops[0] = tmp1;   
            code2->ops[1] = tmp2;  
            // 合并中间代码
    ...
    */
        ...
        // 将place初始赋值为0 即条件表达式初始值置为假
        InterCode code0 = (InterCode)malloc(sizeof(InterCode_));
        code0->kind = ASSIGN_IR;
        code0->ops[0] = place;
        code0->ops[1] = getValue(0);
        // 调用translateCond函数翻译条件表达式 生成中间代码
        InterCode code1 = translateCond(root, label1, label2);
        // 优化：如果code1的最后一句是LABEL语句，那么将code1中的所有GOTO语句中的该LABEL替换为LABEL1
        optimizeLABELBeforeGOTO(code1, label1); 
        insertInterCode(code1, code0);
        // label1标签 中间代码
        InterCode code_label1 = (InterCode)malloc(sizeof(InterCode_));
        code_label1->kind = LABEL_IR;
        code_label1->ops[0] = label1;
        insertInterCode(code_label1, code0);
        // 条件为真时，将place的值设置为1
        InterCode code2 = (InterCode)malloc(sizeof(InterCode_));
        code2->kind = ASSIGN_IR;
        code2->ops[0] = place;
        code2->ops[1] = getValue(1);
        insertInterCode(code2, code0);
        // label2标签 中间代码
        InterCode code_label2 = (InterCode)malloc(sizeof(InterCode_));
        code_label2->kind = LABEL_IR;
        code_label2->ops[0] = label2;
        insertInterCode(code_label2, code0);
        return code0; 
    }
...
InterCode translateCond(Node* root, Operand labelTrue, Operand labelFalse) {
    ...
    // 处理关系运算符(RELOP)表达式
    else if (root->childNum >= 2 && strcmp(root->children[1]->name, "RELOP") == 0) {
        // 创建临时变量tmp1和tmp2存储左右两个表达式的值
        Operand t1 = newTemp();
        Operand t2 = newTemp();
        // 进行表达式的翻译 计算表达式的值并存储
        ...
        // 创建条件跳转指令 条件满足则跳转到labelTrue
        InterCode code_ifgoto = (InterCode)malloc(sizeof(InterCode_));
        ...
        // 条件不满足 无条件跳转到labelFalse
        InterCode code_goto = (InterCode)malloc(sizeof(InterCode_));
        code_goto->kind = GOTO_IR;
        code_goto->ops[0] = labelFalse;
        insertInterCode(code_goto, code1);
        return code1;
    }
    // 处理逻辑与(AND)表达式
        ...
        InterCode code1;
        // AND 先翻译左子表达式 如果为假则直接跳转到labelFalse  否则继续处理右子表达式
        code1 = translateCond(root->children[0], label, labelFalse);
        InterCode code_label = (InterCode)malloc(sizeof(InterCode_));
        code_label->kind = LABEL_IR;
        code_label->ops[0] = label;
        insertInterCode(code_label, code1);
        // 翻译右子表达式 根据其值决定是否跳转到labelTrue或labelFalse
    ...
    // 处理逻辑或(OR)表达式
        ...
        Operand label = newLabel(); 
        InterCode code1;
        // OR  先翻译左子表达式 如果为真则直接跳转到labelTrue  否则继续处理右子表达式
    ...
    // 视为单一表达式进行处理
        ...
        Operand t1 = newTemp();
        // 进行表达式的翻译 并存储值
        InterCode code = translateExp(root, t1);
        // 生成跳转指令 判断是否跳到labelTrue
        InterCode code_ifgoto = (InterCode)malloc(sizeof(InterCode_));
        code_ifgoto->kind = IF_GOTO_IR;
        code_ifgoto->ops[0] = t1;
        code_ifgoto->ops[1] = getValue(0); // 不等于0则进行true标签的跳转
        code_ifgoto->ops[2] = labelTrue;
        strcpy(code_ifgoto->relop, "!=");
        insertInterCode(code_ifgoto, code);
        // 条件为假 无条件跳到labelFalse
        InterCode code_goto = (InterCode)malloc(sizeof(InterCode_));
        code_goto->kind = GOTO_IR;
        code_goto->ops[0] = labelFalse;
        insertInterCode(code_goto, code);
        return code;
    }
}
