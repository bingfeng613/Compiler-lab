...
Operand newTemp()
{
    ...
    tmpVar->no = tmpVarNo;
    // 编号在每次函数调用后递增 确保每个临时变量都有一个唯一的标识
    tmpVarNo++;
    return tmpVar;
}
    ...
    if (root->childNum == 3 && strcmp(root->children[1]->name, "ASSIGNOP") == 0)
    {
        ...
        */
            ...
            strcmp(root->children[0]->children[0]->name, "ID") == 0)
        {
            ...
            // 创建新的指令 将tmp2的值赋给tmp1
            InterCode code2 = (InterCode)malloc(sizeof(InterCode_));
            code2->kind = ASSIGN_IR; // 指令类型
            code2->ops[0] = tmp1;
            code2->ops[1] = tmp2;
            insertInterCode(code2, code1); // 插入指令到双向链表中
            // 将运算结果存回place
            if (place != NULL)
                operandCpy(place, tmp1);
            // 返回code1 生成的代码被插入到code1链表中
            return code1;
        }
        // 数组元素作为左值
                 ...
                 strcmp(root->children[0]->children[1]->name, "LB") == 0)
        {
            ...
            // 进行赋值
            // code6 创建新的中间代码指令，用于将 tmp5 中的值赋给 tmp4 所指向的数组元素位置。
            // 这里使用的中间代码类型是 TO_MEM_IR，表示赋值操作。
            code6->kind = TO_MEM_IR;
            code6->ops[0] = tmp4;
            code6->ops[1] = tmp5;
            // 插入到中间代码链表中 保证代码执行的正确顺序
            insertInterCode(code2, code1); // 插入指令到双向链表中
            ...
            // 返回code1 因为所有新生成的代码都被插入到code1的链表中
            return code1;
        }
        // 结构体特定域作为左值
